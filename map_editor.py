...
while not done:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.VIDEORESIZE:
            ...
            if event.button == 1:
                if state.moving_camera:
                    ...
                    for i, point in enumerate(level.points):
                        if (event.pos[0]+state.camera_shift[0]-point[0])**2+(event.pos[1]+state.camera_shift[1]-point[1])**2<state.point_size**2+1:
                            state.dragging_point_id = i
                            break
                    else: # this is for-else expression, not if-else
                        level.points.append(snap_to_grid((event.pos[0]+state.camera_shift[0],event.pos[1]+state.camera_shift[1])))
            ...
            if state.dragging_point_id is not None:
                level.points[state.dragging_point_id] = snap_to_grid((event.pos[0]+state.camera_shift[0],event.pos[1]+state.camera_shift[1]))
    ...
    for i, point in enumerate(level.points):
        if i == state.dragging_point_id:
            ...
            col = WHITE
        cam_point = (point[0]-state.camera_shift[0],
                     point[1]-state.camera_shift[1])
        pygame.draw.circle(screen, col, cam_point, state.point_size)
